    ...
    <footer className="border-t-2 border-fg bg-fg text-white">
      <div className="mx-auto max-w-7xl">
        <div className="flex items-center justify-between border-b-2 border-white/20 px-4 py-4 sm:px-6">
          <Logo tono="claro" />
          <p className="hidden font-mono text-[9px] text-white/30 sm:block">
            Guías, herramientas y datos para viajar a Japón
          </p>
        </div>
          ...
          {COLS.map((col) => (
            <div key={col.title} className="p-4 sm:p-6 lg:p-8">
              ...
              <ul className="space-y-2">
                {col.links.map((l) => (
                  <li key={l.label}>
                    <Link href={l.href} className="text-xs text-white/60 transition-colors hover:text-white sm:text-sm">
                      {l.label}
                    </Link>
                  </li>
                ))}
